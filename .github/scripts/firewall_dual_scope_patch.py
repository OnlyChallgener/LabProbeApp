...
path = Path('app/src/main/kotlin/com/labprobe/app/RouterControlUi.kt')
text = path.read_text(encoding='utf-8')
start = text.index('@Composable\nfun RouterFirewallScreen')
end = text.index('\n@Composable\nprivate fun FirewallRuleCard', start)
...
text = text.replace('    draggable: Boolean,\n    dragging: Boolean,', '    draggable: Boolean,\n    dragEnabled: Boolean,\n    dragging: Boolean,', 1)
old = '''                    if (draggable) {
...
if 'versionCode = 228' not in build or 'versionName = "0.10.73"' not in build:
    raise SystemExit('unexpected app version baseline')
build = build.replace('versionCode = 228', 'versionCode = 229', 1).replace('versionName = "0.10.73"', 'versionName = "0.10.74"', 1)
