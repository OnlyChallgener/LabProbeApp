        ...
        Surface(
            ...
            Box(Modifier.fillMaxSize()) {
                Canvas(
                    ...
                    val rawMin = visibleValues.minOrNull() ?: minFloor ?: 0.0
                    val rawMax = visibleValues.maxOrNull() ?: (minFloor ?: 1.0) + 1.0
                    val range = (rawMax - rawMin).coerceAtLeast(if (minFloor == null) 3.0 else 5.0)
                    var yMin = rawMin - range * .25
                    var yMax = rawMax + range * .25
                    ...
                    xTicks(visibleStart, visibleEnd).forEach { sec ->
                        ...
                        drawContext.canvas.nativeCanvas.drawText(sec.roundToInt().toString(), x.coerceIn(plotLeft + 8.dp.toPx(), plotRight - 8.dp.toPx()), plotBottom + 15.dp.toPx(), xPaint)
                    }
