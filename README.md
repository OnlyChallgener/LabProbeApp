...
> 💡 **安装升级与跨端协同**：详见全组件指引文档 ➜ [全系统安装与升级完整指南 (App / Hub / Relay)](docs/INSTALL_AND_UPDATE_GUIDE.md)
...
一级入口「网络健康」集中展示互联网、局域网、DNS、IPv6、Router、Relay 与 WireGuard。点击「开始诊断」依次检查九项现有能力，先给出确定性结论，再展开查看带观测点说明的详细结果；未配置、未启用或缺少证据的项目显示「未确认」。

诊断仅在 App 前台运行，可停止；单项失败或超时继续下一项，迟到结果不会覆盖结论。复用现有 Ping、TCP、DNS 及状态接口，不自动修复、不改变连接或规则，也不增加 Hub 协议或后台服务。手机网络与 Router/Relay 的观测结果分开说明，避免把某个探测目标未响应当作整网断线。

现有 AI 助手支持「打开网络健康」「开始网络诊断」「查看最近诊断结果」。解释类问题将最近一次安全诊断摘要提供给既有模型；检测和状态判断始终使用同一套确定性规则。路由状态页的上传、下载和会话趋势继续保留，支持最近 5/15 分钟与长按读数。
...
为避免动态地址冲突，APP 将 profile 明确分为三种 ownership：

- **我的配置（manual）**：用户独立维护 Endpoint；所有自动更新器都会跳过，绝不改写。
- **DDNS 模式**：Endpoint 保持 `域名:固定 UDP 端口`；DDNS 的 A 记录更新由 DNS 解析自然生效。
- **STUN 模式**：Endpoint 只由对应 STUN 规则的最新 `公网 IP:端口` 更新；暂时取不到地址时保留上一条可用地址。

自动更新器会检查 `endpointSource` 与独立的 `endpointRevision`，不能覆盖另一份 profile；用户编辑使用独立的 `profileRevision`。若用户要手动改自动 profile 的地址，APP 要求明确转为手动配置。相同服务端公钥只会显示重复提示，绝不会合并或覆盖。客户端私钥按 profile 用 Android Keystore 加密保存，Hub 不保存或回传私钥。
